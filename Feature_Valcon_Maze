"""
🔥 VOLCANIC MAZE ARENA - COMPLETE STANDALONE GAME 🔥
...
# ========== MAIN ==========
def main():
    print("🔥 VOLCANIC MAZE ARENA 🔥")
    pygame.init()
    W, H, GS = 1200, 800, 25
    screen = pygame.display.set_mode((W, H), pygame.DOUBLEBUF|pygame.OPENGL)
    pygame.display.set_caption("🔥 Volcanic Maze")
    ...
    while run:
        ...
        if ct-last_dmg>0.5 and (lava.check_damage(agx, agz) or obs.dmg(agx, agz)):
            ...
            print(f"💥 HP: {hp}")
            if hp<=0: print("💀 GAME OVER!"); run = False
        ...
        if agent.done:
            print(f"🎉 WIN! Time: {gt:.2f}s | HP: {hp}"); run = False
