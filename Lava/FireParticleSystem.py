...
class Ember:
    ...
    def get_alpha(self) -> float:
        ...
        if life_ratio < 0.1:
            return life_ratio * 10  # ظهور تدريجي
        elif life_ratio > 0.7:
            return (1.0 - life_ratio) / 0.3  # اختفاء تدريجي
        return 1.0


class FireParticleSystem:
    """نظام جزيئات النار المحسّن"""
    ...
    def update(self, dt: float):
        ...
        if self.spawn_points and len(self.particles) < self.max_particles:
            if self.spawn_timer >= 0.05:  # كل 50ms
                self.spawn_timer = 0.0
    ...
    def render(self):
        ...
        # رسم الشرارات واللهب (additive)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE)
        
        for particle in self.particles:
            if particle.ember_type == "ash":
                continue  # الرماد يُرسم لاحقاً
