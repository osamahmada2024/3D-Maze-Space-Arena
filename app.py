...
def main():
    ...
    # Pass Grid to Agent for slippery logic
    color = (random.random(), random.random(), random.random())
    agent = Agent(start, goal, path, 2.0, color, grid=grid)
    
    # PASS particle system to agent (for breath)
    agent.particle_system = particles

    # Provide a helper to convert grid coords -> world coords (used by agent to emit breath)
    agent.grid_to_world = lambda gx, gz: ((gx - GRID_SIZE/2.0) * CELL_SIZE, 0.6, (gz - GRID_SIZE/2.0) * CELL_SIZE)
